...
def correlation_test(samples: np.ndarray, lags: list[int]) -> dict[int, tuple[float, float]]:
...
def run_all_tests(samples: np.ndarray, label: str) -> None:
    ...
    logger.info(f"{label} chi-squared", statistic=round(chi2_stat, 4), p_value=round(chi2_p, 4), reject=chi2_stat > 16.9)

    ks_stat, ks_p = stats.kstest(samples, "uniform")
    logger.info(f"{label} KS", statistic=round(ks_stat, 4), p_value=round(ks_p, 4), reject=ks_stat > 0.0136)

    z, run_p = run_test(samples)
    logger.info(f"{label} runs", z=round(z, 4), p_value=round(run_p, 4), reject=abs(z) > 1.96)

    threshold = 1.96 / np.sqrt(n)
    for h, r in pearson_correlation(samples, LAGS).items():
        logger.info(f"{label} Pearson r_h", lag=h, r=round(r, 4), reject=abs(r) > threshold)

    for h, (c_h, p) in correlation_test(samples, LAGS).items():
        z_h = (c_h - 0.25) / np.sqrt(7 / (144 * (n - h)))
        logger.info(f"{label} c_h test", lag=h, c_h=round(c_h, 6), z=round(z_h, 4), p_value=round(p, 4), reject=abs(z_h) > 1.96)
...
def multi_seed_rejection_rates(n_seeds: int = 1_000) -> None:
    ...
    for seed in range(n_seeds):
        for samp, rej in [
            ...
            if chi2 > 16.9: rej["chi2"] += 1
            ks, _ = stats.kstest(samp, "uniform")
            if ks > 0.0136: rej["ks"] += 1
            z, _ = run_test(samp)
            if abs(z) > 1.96: rej["runs"] += 1
            for key, h in [("ch1", 1), ("ch2", 2), ("ch5", 5)]:
                c_h = float(np.mean(samp[:-h] * samp[h:]))
                z_h = (c_h - 0.25) / np.sqrt(7 / (144 * (N - h)))
                if abs(z_h) > 1.96: rej[key] += 1
    ...
    for lbl, rej in [("Good LCG", lcg_rej), ("Mersenne Twister", mt_rej)]:
        logger.info(
            ...
            chi2=pct(rej, "chi2"), ks=pct(rej, "ks"), runs=pct(rej, "runs"),
            ch_h1=pct(rej, "ch1"), ch_h2=pct(rej, "ch2"), ch_h5=pct(rej, "ch5"),
        )


if __name__ == "__main__":
    # --- Part 1a: Good LCG ---
    logger.info("Part 1a: Good LCG histogram", a=A_GOOD, c=C_GOOD, m=M_GOOD, seed=settings.SEED)
    ...
    with figure(figsize=(6, 6), save=f"{PLOTS}/bad_lcg_scatter.png") as fig:
        ...
        ax.set_xlabel("$U_i$")
        ax.set_ylabel("$U_{i+1}$")
        ax.set_title(f"Bad LCG consecutive pairs — a={A_BAD}, c={C_BAD}, M={M_BAD} (n={N:,})")

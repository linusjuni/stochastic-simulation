...
from exercises.day6.tsp import (
    euclidean_matrix,
    log_cooling,
    reverse_proposal,
    route_cost,
...
N = 20
N_ITER = 20_000
OUT = Path(__file__).resolve().parents[2] / "report" / "plots" / "day6"
...
def part2_cost_matrix(rng: np.random.Generator) -> None:
    D = np.loadtxt(Path(__file__).resolve().parents[0] / "cost.csv", delimiter=",")
    n = D.shape[0]
    ...
    with figure(figsize=(8, 5), save=OUT / "tsp_costmatrix.png") as fig:
        ...
        ax.set_title(
            f"Cost history (best config: {best_config[0]} cooling, {best_config[1]} proposal)"
        )

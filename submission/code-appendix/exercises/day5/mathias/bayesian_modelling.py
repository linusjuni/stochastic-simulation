...
def metropolis_hastings_part2(
    n_samples, init_state_theta, init_state_psi, X_obs, Sigma, thinning=10
):
    ...
    for _ in range(n_samples):
        ...
        acceptance_ratio = min(
            1,
            posterior(proposed_theta, proposed_psi, X_obs, rho)
            / posterior(curr_state_theta, curr_state_psi, X_obs, rho),
        )
...
def metropolis_hastings_log(
    ...
    for _ in range(n_samples):
        ...
        acceptance_ratio = min(
            1,
            np.exp(
                log_posterior(proposed_theta, proposed_psi, X_obs, rho)
                - log_posterior(curr_state_theta, curr_state_psi, X_obs, rho)
            ),
        )
        if np.random.random() < acceptance_ratio:
            samples.append((proposed_theta, proposed_psi))
...
if __name__ == "__main__":
    ...
    samples = metropolis_hastings_log(
        n_samples, init_state_theta=theta, init_state_psi=psi, X_obs=X_obs, Sigma=Sigma
    )
    burn_in = int(0.1 * len(samples))
    theta_s = [s[0] for s in samples[burn_in:]]

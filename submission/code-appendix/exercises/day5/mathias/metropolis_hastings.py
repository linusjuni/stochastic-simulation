...
def metropolis_hastings_part1(n_samples, initial_state=5, lo=0, hi=10, thinning=10):
    ...
    for _ in range(n_samples):
        ...
        acceptance_prob = min(
            1,
            joint_density_rvs_part1(proposed_state)
            / joint_density_rvs_part1(current_state),
        )
...
def metropolis_hastings_part2(
    n_samples, init_state_1=5, init_state_2=5, lo=0, hi=10, thinning=10
):
    ...
    for _ in range(n_samples):
        ...
        if p < 0.5:
            proposed_i, proposed_j = (
                curr_state_i + np.random.choice([-1, 1]),
                curr_state_j,
            )
        else:
            proposed_i, proposed_j = (
                curr_state_i,
                curr_state_j + np.random.choice([-1, 1]),
            )
        if (
            proposed_i < lo
            or proposed_i > hi
            or proposed_j < lo
            or proposed_j > hi
            or proposed_i + proposed_j > hi
        ):
            samples.append((curr_state_i, curr_state_j))
            continue
        acceptance_prob = min(
            1,
            joint_density_rvs_part2(proposed_i, proposed_j)
            / joint_density_rvs_part2(curr_state_i, curr_state_j),
        )
        p = np.random.random()
...
if __name__ == "__main__":
    os.makedirs("report/plots/day5", exist_ok=True)
    n_samples = 100000
    lo, hi = 0, 10
    initial_state = 0
    ...
    histogram(
        samples,
        discrete=True,
        title="Metropolis-Hastings Samples, A = 8, m=10",
        xlabel="State",
        ylabel="Frequency",
    )
    ...
    histogram(
        erlang.rvs(a=8, size=100000),
        discrete=True,
        title="Samples from Erlang Distribution (A=8)",
        xlabel="State",
        ylabel="Frequency",
    )
    plt.show()
    ...
    histogram(
        [i + j for i, j in samples],
        discrete=True,
        title="Metropolis-Hastings Samples, A1 = A2 = 4, m=10",
        xlabel="State",
        ylabel="Frequency",
    )
    ...
    histogram(
        [i + j for i, j in samples],
        discrete=True,
        title="Gibbs Sampling Samples, A1 = A2 = 4, m=10",
        xlabel="State",
        ylabel="Frequency",
    )
